...
ImpiantoTropicale::ImpiantoTropicale(int id, const string& nome) //costruttore: inizializza l’impianto con consumo di 0.5 litri/minuto
...
bool ImpiantoTropicale::impostaTimer2(const Orario& inizio) { //imposta il timer con un solo orario
    orarioTimer = inizio; //salva orario di inizio
    logMessage(inizio, " Timer impostato per \"" + nome + "\"", 0);
    return true;
}

bool ImpiantoTropicale::rimuoviTimer() { //rimuove il timer se presente
...
void ImpiantoTropicale::aggiornaStato(const Orario& orarioPrecedente, const Orario& orarioCorrente) { //aggiorna lo stato dell’impianto in base all’orario
    ...
    if (dovrebbeEssereAttivoOra != dovrebbeEssereStatoAttivo) { //se lo stato è cambiato
        ...
        aggiornaConsumo(minutiAttivi); //calcola tempo trascorso
    }
}

string ImpiantoTropicale::getTipo() const { //restituisce una stringa con tipo impianto
    return "Tropicale";
}

bool ImpiantoTropicale::dovrebbeEssereAttivo(const Orario& orario) const { //controlla se l’impianto dovrebbe essere acceso all’orario indicato
...
Orario ImpiantoTropicale::calcolaOrarioSpegnimento() const { //calcola orario di spegnimento: 2h e 30 dopo attivazione
