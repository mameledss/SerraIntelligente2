...
ImpiantoAlpino::ImpiantoAlpino(int id, const string& nome) //costruttore: inizializza l’impianto con consumo di 0.5 litri/minuto
...
bool ImpiantoAlpino::impostaTimer2(const Orario& inizio) { //imposta il timer con un solo orario
    orarioTimer = inizio; //salva orario di inizio
    logMessage(inizio, " Timer impostato per \"" + nome + "\"", 0);
    return true;
}

bool ImpiantoAlpino::rimuoviTimer() { //rimuove il timer se presente
...
void ImpiantoAlpino::aggiornaStato(const Orario& orarioPrecedente, const Orario& orarioCorrente) { //aggiorna lo stato dell’impianto in base all’orario
...
string ImpiantoAlpino::getTipo() const { //restituisce una stringa con tipo impianto
    return "Alpino";
}

bool ImpiantoAlpino::dovrebbeEssereAttivo(const Orario& orario) const { //controlla se l’impianto dovrebbe essere acceso all’orario indicato
...
Orario ImpiantoAlpino::calcolaOrarioSpegnimento() const { //calcola orario di spegnimento: 2h e 30 dopo attivazione
